...
const sudokuReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ACTIONS.LOAD_RANDOM:
      ...
      return {...state, type: newType, fields: newFields, solution, sudokuIndex: newIndex, timer: 0, gameState: 'running', history: [], mistakes: 0};
    // loads one of the stored sudokus
    case ACTIONS.LOAD: {
      const fields = SUDOKUS[type][action.payload.sudokuIndex];
      const solution = solve(fields);
      return {...state, fields, solution, sudokuIndex: action.payload.sudokuIndex, timer: 0, history: [], mistakes: 0};
    }
    // clears all user selected fields. resets to the current sudoku
    case ACTIONS.RESET:
      if (!type || type === 'custom') {
        return {...state, type: 'custom', fields: Array(81).fill(0), sudokuIndex: null, timer: 0, history: [], mistakes: 0};
      }
      return {...state, fields: SUDOKUS[type][sudokuIndex], sudokuIndex: sudokuIndex, timer: 0, history: [], mistakes: 0};
    // clears all fields
    case ACTIONS.CLEAR:
      return {...state, fields: Array(81).fill(0), solution: [], sudokuIndex: null, timer: 0, history: [], mistakes: 0};
    // solve the sudoku
    case ACTIONS.SOLVE:
    ...
    case ACTIONS.SET_FIELD: {
      ...
      const mistakes = state.solution[action.payload.clicked] === action.payload.value ? state.mistakes : state.mistakes + 1;
...
function Sudoku() {
  ...
  const initialSudoku = () => {
    ...
    return {type, fields, sudokuIndex, gameState, timer, history, mistakes, solution};
  };

  const [{type, fields, sudokuIndex, gameState, timer, history, mistakes, solution}, dispatch] = React.useReducer(
    sudokuReducer,
    undefined,
    initialSudoku
  );
  ...
  useEffect(() => window.localStorage.setItem('mistakes', JSON.stringify(mistakes)), [mistakes]);
